...
public class AccelerometerInput4 : MonoBehaviour
{
	...
	void FixedUpdate () //was previously FixedUpdate()
	{
		...
		// float y = average ();
		float y = display.acceleration.y;
		float z = display.acceleration.z;
		float ex = InputTracking.GetLocalRotation (XRNode.Head).eulerAngles.x;
		float ey = InputTracking.GetLocalRotation (XRNode.Head).eulerAngles.y;
		float ez = InputTracking.GetLocalRotation (XRNode.Head).eulerAngles.z;
		Debug.Log ("ACCELERATION_XYZ: " + x + " " + y + " " + z + " EULER_ANGLES_XYZ " + ex + " " + ey + " " + ez + " " + looking.ToString ());
